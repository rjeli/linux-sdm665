...
static int disp_cc_sm6125_probe(struct platform_device *pdev)
{
	...
	*/
	...
	/*
	clk = devm_clk_get(&pdev->dev, "cfg_ahb");
	if (IS_ERR(clk)) {
	...
	*/
	...
	ret = qcom_cc_really_probe(pdev, &disp_cc_sm6125_desc, regmap);
	if (ret) {
...
MODULE_DESCRIPTION("QTI DISP_CC sm6125 Driver");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("platform:disp_cc-sm6125");
